...
class Setup extends BaseCommand
{
    ...
    private function setupPhone(): void
    {
        $file = 'Config/Auth.php';
        ...
        $smsSecretToken = (string) $config->smsSecretToken; // Old Config may return null.

        if ($smsSecretToken !== '') {
            $this->write(CLI::color('  Phone Setup: ', 'green') . 'Everything is fine.');
        ...
        if ($smsSecretToken === '') {
            $set = $this->prompt('  Do you set smsSecretToken now?', ['y', 'n']);

            if ($set === 'y') {
                // Input from phone
                $smsSecretToken = $this->prompt('  What is your SMS Secret Token?', null, 'required|alpha_numeric_punct');

                $pattern = '/^    public .*\$smsSecretToken\s+= \'\';/mu';
                $replace = '    public string $smsSecretToken  = \'' . $smsSecretToken . '\';';
