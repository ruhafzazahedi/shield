...
class User extends Entity
{
     ...
     *                     'phone_2fa'|'phone_activate'|'phone_password'|'magic-link'|'access_token'
     */
    ...
    /**
     * Creates a new identity for this user with an phone/password
     * combination.
     *
     * @phpstan-param array{phone: string, password: string} $credentials
     */
    public function createPhoneIdentity(array $credentials): void
    {
        /** @var UserIdentityModel $identityModel */
        $identityModel = model(UserIdentityModel::class);
    ...
    /**
     * Returns the user's Phone/Password identity.
     */
    public function getPhoneIdentity(): ?UserIdentity
    {
        return $this->getIdentity(Session::ID_TYPE_PHONE_PASSWORD);
    }

    /**
     * If $phone, $password, or $password_hash have been updated,
     * will update the user's phone identity record with the
     * correct values.
     */
    public function savePhoneIdentity(): bool
    {
        if (empty($this->phone) && empty($this->password) && empty($this->password_hash)) {
            return true;
        }
    ...
    /**
     * Accessor method to grab the user's phone address.
     * Will cache it in $this->phone, since it has
     * to hit the database the first time to get it, most likely.
     */
    public function getPhone(): ?string
    {
        if ($this->phone === null) {
            $this->phone = $this->getPhoneIdentity()->secret ?? null;
        }
    ...
    public function getPasswordHash(): ?string
    {
        if ($this->password_hash === null) {
            $this->password_hash = $this->getPhoneIdentity()->secret2 ?? null;
        }
